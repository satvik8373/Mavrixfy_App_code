...
export default function PlaylistScreen() {
  ...
  useEffect(() => {
    ...
    if (firestore === "true") {
      ...
      setLoading(true);
      // Call JioSaavn API directly like the web does
      const apiUrl = "https://saavn.sumit.co/api";
      const url = `${apiUrl}/playlists?id=${id}`;
      console.log("=== Loading JioSaavn Playlist ===");
      ...
      fetch(url)
        ...
        .then((json: any) => {
          ...
          // JioSaavn API returns { success: true, data: {...} }
          if (json.success && json.data) {
            const data = json.data;
            console.log("Has songs?", !!data.songs);
            console.log("Songs count:", data.songs?.length || 0);
            ...
            console.log(`Successfully converted ${converted.length} songs`);
            setSongs(converted);
          } else {
            console.error("Invalid response - no success or data");
            setNotFound(true);
          }
        })
        .catch((error) => {
  ...
  const handlePlayAll = () => {
    ...
    const isPlayingFromThisPlaylist = songs.some(s => s.id === currentSong?.id) &&
      queue.length === songs.length &&
      queue.every((q, i) => q.id === songs[i]?.id);

    if (isPlayingFromThisPlaylist) {
      // Toggle play/pause if already playing this playlist
      togglePlay();
    } else {
