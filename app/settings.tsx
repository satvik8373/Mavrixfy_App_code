...
export default function SettingsScreen() {
  ...
  const clearRecentlyPlayed = () => {
    Alert.alert("Clear Recently Played", "Are you sure?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Clear",
        style: "destructive",
        onPress: async () => {
          await AsyncStorage.removeItem("@mavrixfy_recently_played");
    ...
    <View style={[styles.container, { paddingTop: topInset }]}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.presetsWrap}>
            {PRESET_NAMES.map((name) => (
              <Pressable
                ...
                style={[
                  styles.presetBtn,
                  activePreset === name && styles.presetBtnActive,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.presetText,
                    activePreset === name && styles.presetTextActive,
                  ]}
                >
                  {name}
                </Text>
          ...
          <View style={styles.eqContainer}>
            {FREQ_BANDS.map((band, idx) => {
              const db = settings.equalizer[band] ?? 0;
              return (
                <Pressable
                  key={band}
                  style={styles.eqBand}
                  onPress={() => cycleBandValue(band)}
                >
                  <Text style={styles.eqDbLabel}>
                    {db > 0 ? `+${db}` : db}
                  </Text>
                  ...
                  <Text style={styles.eqFreqLabel}>{band}</Text>
                </Pressable>
              );
            })}
          </View>
        ...
        <View style={styles.section}>
          ...
          <Pressable style={styles.dangerBtn} onPress={clearRecentlyPlayed}>
            <Ionicons name="time-outline" size={18} color="#FF6B6B" />
            <Text style={styles.dangerText}>Clear Recently Played</Text>
          </Pressable>
